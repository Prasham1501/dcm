...
function ris_render_srs(array $visit, array $orders): void
{
    $rows = '';
    foreach ($orders as $order) {
        // Accession is intentionally hidden from reception printouts.
        $rows .= '<tr><td>' . ris_h($order['service_name'] ?: '-') . '</td><td>' . ris_h($order['sample_type'] ?: '-') . '</td><td>' . ris_h($order['tube_type'] ?: '-') . '</td><td>' . ris_h((string)($order['tube_count'] ?: 1)) . '</td></tr>';
    }
    echo '<!doctype html><html><head><meta charset="utf-8"><title>SRS ' . ris_h($visit['visit_no']) . '</title>' . ris_print_css() . '</head><body><div class="page"><div class="top"><div><div class="brand">Sample Receipt Slip</div><div class="muted">' . ris_h($visit['visit_no']) . '</div></div><div class="muted">' . ris_h($visit['visit_datetime']) . '</div></div><div class="grid"><div class="box"><div class="label">Patient</div><div class="value">' . ris_h($visit['full_name']) . '</div><div class="muted">MRN ' . ris_h($visit['mrn']) . ' | ' . ris_h($visit['phone'] ?: '-') . '</div></div><div class="box"><div class="label">Collection</div><div class="value">' . ris_h($visit['center_name'] ?: 'Main Lab') . '</div><div class="muted">Staff: ' . ris_h($visit['phlebotomy_staff'] ?: '-') . ' | Area: ' . ris_h($visit['home_visit_area'] ?: '-') . ' | Home visit Rs ' . number_format((float)($visit['home_visit_amount'] ?? 0), 2) . '</div></div></div><table><thead><tr><th>Service</th><th>Sample</th><th>Tube</th><th>Qty</th></tr></thead><tbody>' . $rows . '</tbody></table><table class="totals" style="margin-left:auto;width:300px"><tr><td>Total</td><td class="num">Rs ' . number_format((float)$visit['net_amount'], 2) . '</td></tr><tr><td>Paid</td><td class="num">Rs ' . number_format((float)$visit['paid_amount'], 2) . '</td></tr><tr><th>Balance</th><th class="num">Rs ' . number_format((float)$visit['balance'], 2) . '</th></tr></table><p class="noprint"><button onclick="window.print()">Print SRS</button></p></div></body></html>';
    exit;
}
...
function ris_render_bill_receipt(array $visit, array $orders, array $payments): void
{
    ...
    foreach ($orders as $order) {
        // Accession is intentionally hidden from reception printouts.
        $orderRows .= '<tr><td>' . ris_h($order['service_name'] ?: '-') . '</td><td class="num">Rs ' . number_format((float)$order['price'], 2) . '</td></tr>';
    }
    $payRows = '';
    ...
    echo '<!doctype html><html><head><meta charset="utf-8"><title>Bill ' . ris_h($visit['visit_no']) . '</title>' . ris_print_css() . '</head><body><div class="page"><div class="top"><div><div class="brand">Bill Receipt</div><div class="muted">' . ris_h($visit['visit_no']) . '</div></div><div class="muted">' . ris_h($visit['visit_datetime']) . '</div></div><div class="grid"><div class="box"><div class="label">Patient</div><div class="value">' . ris_h($visit['full_name']) . '</div><div class="muted">MRN ' . ris_h($visit['mrn']) . ' | ' . ris_h($visit['phone'] ?: '-') . '</div></div><div class="box"><div class="label">Doctor / Center</div><div class="value">' . ris_h($visit['doctor_name'] ?: 'Self') . '</div><div class="muted">' . ris_h($visit['center_name'] ?: 'Main Lab') . '</div><div class="muted">Home visit: ' . ris_h(((float)($visit['home_visit_amount'] ?? 0) > 0 || ($visit['home_visit_area'] ?? '') !== '') ? (($visit['home_visit_area'] ?: '-') . ' | Rs ' . number_format((float)$visit['home_visit_amount'], 2)) : 'No') . '</div></div></div><table><thead><tr><th>Service</th><th class="num">Amount</th></tr></thead><tbody>' . $orderRows . '</tbody></table><table><tr><td>Total</td><td class="num">Rs ' . number_format((float)$visit['total_amount'], 2) . '</td></tr><tr><td>Home visit</td><td class="num">Rs ' . number_format((float)($visit['home_visit_amount'] ?? 0), 2) . '</td></tr><tr><td>Extra charge</td><td class="num">Rs ' . number_format((float)$visit['misc_charge'], 2) . '</td></tr><tr><td>Discount</td><td class="num">Rs ' . number_format((float)$visit['discount'], 2) . '</td></tr><tr><th>Final</th><th class="num">Rs ' . number_format((float)$visit['net_amount'], 2) . '</th></tr><tr><td>Paid</td><td class="num">Rs ' . number_format((float)$visit['paid_amount'], 2) . '</td></tr><tr><td>Balance</td><td class="num">Rs ' . number_format((float)$visit['balance'], 2) . '</td></tr></table><h3>Payments</h3><table><thead><tr><th>Mode</th><th>Paid by</th><th>Date</th><th class="num">Amount</th></tr></thead><tbody>' . ($payRows ?: '<tr><td colspan="4">No payments recorded</td></tr>') . '</tbody></table><p class="noprint"><button onclick="window.print()">Print bill receipt</button></p></div></body></html>';
    exit;
}
