/**
 * Layout catalog — copied from dcm/www/src/types/viewer.ts so the picker
 * shows the SAME layouts the DCM viewer offers.
 */
...
export function getAllLayouts(): ViewerLayout[] {
  return LAYOUT_CATEGORIES.flatMap((c) => c.layouts);
}

export function findLayoutById(id: string): ViewerLayout | null {
