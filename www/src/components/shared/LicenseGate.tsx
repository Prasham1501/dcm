...
import { Shield, Key, AlertTriangle, CheckCircle, Loader2, Clock, WifiOff } from 'lucide-react';
import { QRCodeSVG } from 'qrcode.react';

// Same REASONS map RechargeTab uses, so error wording is consistent.
...
/**
 * Collapsible "No internet? Activate offline" panel.
 *
 * The PC reads a 7-char Request code (HMAC-bound to the machine fingerprint)
 ...
 * — no network call — and creates a server-less licence with the granted
 * prints + days when redeemed on a trial/no-licence machine.
 */
function OfflineActivationPanel({ onActivated }: { onActivated: () => void }) {
  ...
  return (
    <div className="mt-4 rounded-lg border border-gray-800 bg-gray-900/40">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="flex w-full items-center justify-between gap-2 px-3 py-2 text-xs font-semibold text-gray-300 hover:text-white"
      >
        ...
        <div className="space-y-3 border-t border-gray-800 px-3 py-3">
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              <code className="flex-1 truncate rounded border border-gray-700 bg-gray-800 px-2 py-1.5 text-base font-bold tracking-widest text-amber-300">
                {requestCode || '…'}
              </code>
              <button
                type="button"
                onClick={copyRequest}
                className="rounded border border-gray-700 bg-gray-800 px-2 py-1.5 text-[11px] text-gray-300 hover:bg-gray-700"
              >
                {copied ? 'Copied' : 'Copy'}
              </button>
            </div>
            {requestCode && (
              <div className="mt-2 flex items-center gap-3">
                ...
                <p className="text-[10px] text-gray-500">…or send a photo of this QR — the provider can upload it instead of typing.</p>
              </div>
            )}
          </div>
          ...
          <div>
            ...
            <input
              ...
              maxLength={23}
              spellCheck={false}
              className="w-full rounded border border-gray-700 bg-gray-800 px-2 py-1.5 font-mono text-xs text-white placeholder:text-gray-600 focus:border-amber-500 focus:outline-none"
            />
          </div>
          ...
          <div>
            ...
            <input
              ...
              className="w-full rounded border border-gray-700 bg-gray-800 px-2 py-1.5 font-mono text-xs uppercase tracking-widest text-white placeholder:text-gray-600 focus:border-amber-500 focus:outline-none"
            />
          </div>

          {err && (
            <div className="rounded border border-red-800/40 bg-red-950/20 px-2 py-1.5 text-[11px] text-red-400">
              {err}
            </div>
          )}
