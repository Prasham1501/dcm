...
export function CRPrintPreview({ onClose, initialPageMode = 'all' }: CRPrintPreviewProps) {
  ...
  const buildPrintHtml = useCallback((pagesToPrint: number[]) => {
    ...
    return `<!DOCTYPE html><html><head><meta charset="utf-8"><title>DICOM Print - ${patientName}</title><style>@page{size:${localPaperSize} ${isLandscape ? 'landscape' : 'portrait'};margin:${marginMm}mm}*{box-sizing:border-box}html,body{margin:0;padding:0;font-family:Arial,Helvetica,sans-serif;background:${pageBg};width:${pageW}mm}.page{page-break-after:always;page-break-inside:avoid;display:flex;flex-direction:column;width:${pageW}mm;height:${pageH}mm;overflow:hidden}.page:last-child{page-break-after:auto}.grid{display:grid;grid-template-columns:${grid.columns};grid-template-rows:${grid.rows};${grid.areas ? `grid-template-areas:${grid.areas};` : ''}gap:2px;padding:2px;flex:1;min-height:0;background:#374151}img{display:block;image-rendering:-webkit-optimize-contrast;image-rendering:high-quality}@media print{body{-webkit-print-color-adjust:exact;print-color-adjust:exact}}</style></head><body>${pagesHtml}</body></html>`;
                  ...
                  <div className="flex-1 min-h-0">
                    <div style={previewGridStyle} className="h-full">
                      {Array.from({ length: currentLayout.spots }).map((_, i) => {
                        ...
                        return (
                          <div key={i} className="bg-black overflow-hidden" style={{ ...areaStyle, boxShadow: hospitalConfig.printBorderEnabled ? `inset 0 0 0 1px ${previewBorderCol}` : 'none', padding: 3 }}>
