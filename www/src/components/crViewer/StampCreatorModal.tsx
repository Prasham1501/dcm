/**
 * StampCreatorModal — Create and manage stamps for the CR viewer.
 * Allows creating custom stamps with text, color, and font size.
 */
...
export function StampCreatorModal({ onClose }: StampCreatorModalProps) {
  const { stamps, addStamp, removeStamp } = useCRViewerStore();
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
      <div className="bg-app-bg border border-app-border rounded-lg shadow-2xl w-[480px] max-h-[80vh] flex flex-col">
        ...
        <div className="px-4 py-3 border-b border-app-border">
          ...
          <div className="flex items-end gap-3 mt-2">
            ...
            <div className="flex-1 flex items-center justify-center h-8 bg-black rounded border border-app-border overflow-hidden">
              <span
                style={{ color, fontSize: `${Math.min(fontSize, 20)}px`, fontWeight: 'bold' }}
        ...
        <div className="flex-1 overflow-auto px-4 py-2">
          ...
          {stamps.length === 0 ? (
            <p className="text-xs text-app-text-muted text-center py-4">
              No stamps created yet. Create one above.
            </p>
          ) : (
            <div className="space-y-1">
              {stamps.map((stamp) => (
                <div
                  key={stamp.id}
                  className="flex items-center gap-2 px-2 py-1.5 rounded border border-app-border hover:bg-app-hover"
                >
                  <span
