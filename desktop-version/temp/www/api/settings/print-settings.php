...
try {
    if ($method === 'GET') {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            // Decode JSON fields
            if (in_array($key, ['report_header_layout', 'hospital_info', 'report_footer_content', 'custom_header_text'])) {
    ...
    elseif ($method === 'POST') {
        ...
        foreach ($input as $key => $value) {
            ...
            // Special handling for JSON fields or strings that shouldn't be booleans
            if (in_array($key, ['report_header_layout', 'hospital_info', 'report_footer_content'])) {
                $settingValue = is_array($value) ? json_encode($value) : $value;
            } else {
                // Don't convert font size or color or custom text or footer layout or patient location to boolean
                if (in_array($key, ['header_font_size', 'header_text_color', 'custom_header_text', 'report_footer_layout', 'patient_info_location'])) {
                     $settingValue = isset($value) ? (string)$value : '';
                } else {
                     $settingValue = is_bool($value) ? ($value ? 'true' : 'false') : (string)$value;
                }
            }
